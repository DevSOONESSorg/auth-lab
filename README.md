...
### 教材用の初期ユーザー（パスワードはすべて `Taiken-2026`）

| ログインID | 表示名 | 役割 | 状態 |
|---|---|---|---|
| `admin` | 情シス 新垣 | 情シス（管理者） | 通常 |
| `higa` | 比嘉 | 一般社員 | 通常 |
| `kinjo` | 金城 | 一般社員 | 初期パスワード状態（ログインすると変更を求められる） |

**これは教材用のダミーです。実際のシステムでこのような共通パスワードを使ってはいけません。**

> 補足：`password` のような有名な文字列にすると、Chrome が「このパスワードは漏えいしています」という警告を出して学習の妨げになるため、教材固有の文字列にしています。それでも「パスワードを保存しますか？」と聞かれたら「使用しない」で構いません。
